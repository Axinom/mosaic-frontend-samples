...
import { ROPCSignInResponseCode } from '@axinom/mosaic-user-auth-utils';
...
export const SignInWithAxAuth: React.FC = () => {
  ...
  const { isDirectSignInConfigured, ropcSignIn, getToken } = useUserService();
  ...
  const signIn = async (
    ...
    if (!/\S+@\S+\.\S+/.test(email)) {
      ...
      if (ropcSignInResponse.code === ROPCSignInResponseCode.SUCCESS) {
        ...
        logger.error(`calling [signIn]`, 'output:', ropcSignInResponse);
      }
    }
  };
